...
# Also, don't install local-storage, the metrics-server, or traefik: we don't
# need them, so let's not waste the resources.
k3d cluster create $CLUSTER \
    -p "80:80@loadbalancer" -p "443:443@loadbalancer" \
    --k3s-arg '--disable=local-storage,metrics-server,traefik@server:*;agents:*'
